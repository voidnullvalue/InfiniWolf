...
### Round 3 (2026-07-12, overnight session): macro composition, a real correctness bug, and a secrets vocabulary

The first two passes fixed room silhouettes and local realization; measurement afterward showed the *macro* composition still read as scattered boxes rather than an authored floor plan — real maps have a legible critical path, deliberate loops, a hierarchy of anchor/standard/closet rooms, and recognizable districts. This pass separated **planning** from **realization**: a `FloorPlan` (a spine of role-tagged rooms — start/beat/climax/relief/exit — with a bounded, composable motif grammar) is chosen first, deterministically from the floor's own rng, and only then realized into actual `Room` rectangles and corridors. Motifs compose rather than copy: `ring` (a genuine reconvergent loop, not a shortcut that shrinks the critical path away), `hub` (one high-degree anchor room with satellites), `wings` (a mirrored branch pair), and `gallery` (a chain of same-sized closets) — capped at 3 motifs per floor, each used at most once, so repetition stays a deliberate accent rather than wallpaper. Districts (2–3 contiguous spine segments) drive wall theming so adjacent rooms share a material family and read as a wing rather than a random patchwork.

9. **Structural topology.** Door-bounded rooms/map rose from 16.2 to 22.8 (corpus mean 24.7); doors/map rose from 15.9 to 27.7 (corpus mean 27.2 — close to exact); door-graph cycles (genuine usable loops) rose from a median of 0 to a mean of 1.87 (corpus: mean 1.1–1.54 depending on measurement). A bounded candidate-and-critique loop (`_critique`, generating up to 3 attempts per floor and keeping the one with fewest flags, ties broken by earliest attempt for determinism) checks each finished map for `no_loop`, `no_anchor` (biggest room <10% of floor), `flat_hierarchy` (top-3 rooms <25% of floor), and `corridor_heavy` (>45% of floor outside any room rectangle) — a soft multi-signal vector, deliberately never collapsed into one score, so candidate selection can't be gamed by optimizing a single proxy at the expense of everything else.
10. **A real correctness bug, caught by measurement, not inspection.** The room-placement rewrite initially connected plan-graph nodes with a rock gap (1–6 tiles) plus a routed corridor rather than packing rooms wall-to-wall. Its corridor router tried a "buffered" search first (kept a 1-tile rock margin so a new route can't touch unrelated floor) but, when a crowded hub exhausted the first 40 candidate portal pairs, fell back to an **unbuffered** search that could walk straight through already-carved floor with no wall or door at the seam — silently fusing two rooms into one long open sightline. This was invisible to every existing validator (the map was still fully solvable, doors still fine, secrets still fine) and was only found by directly measuring the longest unobstructed straight floor run across sampled seeds: values of 22–36 tiles turned up with *zero door tiles anywhere on that exact row or column* — proof of a bare seam, not just a long lane the sightline-breaker had declined to touch. The fix: the buffered search now exhausts every portal pair before ever falling back, and the true last-resort fallback now refuses to cross a cell already owned by an unrelated room, raising instead so the existing per-floor retry loop regenerates rather than shipping a fused seam. After the fix, a 45-map sample's longest-run distribution is `14–21` (the report's own hard ceiling), with the *maximum* value across every sample landing at exactly 21, never above. A permanent regression test (`tests/test_topology_regression.py`) now encodes this as a tripwire — both a direct longest-run assertion and an independent "no single door-bounded component should be half the map" check — over a fixed seed list that includes the exact repro, so this bug class can't silently return. The lesson for future rounds: **always verify a structural change by measuring the property it could have broken, not just by re-running existing validators** — validity and quality are different axes, and a bug can live entirely in the gap between them.
11. **Secrets vocabulary.** Every secret was previously an identical 3×2 pocket — exactly the "repetitive one-cell closet" anti-pattern the brief calls out. Secrets are now chosen per-slot from four weighted, seed-deterministic variants: `closet` (0.40, unchanged baseline), `vault` (0.25, a larger 4–5×3 chamber with 2–3 clustered items and a guard as a real risk/reward fight, not a free handout), `nested` (0.20, capped at 1/floor: a lesser outer reward hides a second pushwall to a premium inner reward — validated with a genuinely *sequential* reachability simulation, since the inner pushwall is definitionally unapproachable until the outer one is pushed, which a naive "all pushwalls open at once" check would get wrong), and `shortcut` (0.15, capped at 1/floor: the pocket reconnects to already-carved floor instead of dead-ending, creating a real secret loop; the existing lock-bypass validator, which already tests reachability with every pushwall pushed, transparently enforces that a shortcut can never bypass a lock — no new validation code was needed, just correct geometry). Reward tier is biased by the host room's BFS depth from start (shallow = ammo/low treasure, deep = weapon upgrades/high treasure), so a discovery's value tracks how far the player had to get to earn it. A realized sample lands close to target weights (closet/vault/nested/shortcut ≈ 44/26/18/12% against a 40/25/20/15 target). Two more critique flags close the loop: `long_sightline` (a permanent tripwire for the bug class in point 10 — this flag would have caught that regression automatically, before it needed a manual repro) and `secret_monotony` (a floor with 3+ secrets that all rolled the same variant, a soft nudge for candidate selection rather than a hard rule).
12. **New tooling.** `tools/inspect_map.py` (read-only: renders a floor as ASCII, or parses a real corpus `.wad`/`.pk3`, and prints door-graph topology, room-shape, corridor-share, sightline, and secret-variant metrics for either) makes every claim in this addendum independently reproducible and gives future rounds a single place to check output against the corpus rather than re-deriving metrics ad hoc each time.
