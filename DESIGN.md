...
- **Spine**: approximately 55% of the planned rooms (up to eleven; nine for the enlarged floor 10), always `start → beat* → climax → relief → exit`. This keeps optional wings and motifs while ensuring the mandatory route forms most of the planned progression.
...
Selection is a pure function of the campaign seed: `CampaignConfig.variant_seed(floor)` hashes a dedicated `infiniwolf:variant:v1` payload (independent of `floor_seed` and retry `attempt`), and `_variant_sequence` draws each floor's pick excluding the previous floor's. `CampaignConfig.circulation_seed(floor)` provides a second independent stream; `_circulation_sequence` also excludes the previous skeleton. Retries therefore keep both the floor identity and its high-level building organization. The chosen variant, skeleton, district modes, and compact layout signature are recorded in `GeneratedMap` and the reproducibility manifest.
...
Floor 9 widens its single anchor to 14–17 tiles on each side. Since every other tier is smaller, that anchor is always the selected boss arena; its `grand` treatment adds the intended columned set-piece dressing. The structural predecessor becomes a visible staging room. After encounters and room identities are known, `_place_authored_pickups` gives it a named stock-up composition containing first-aid and ammo, with independent chances for a rare weapon and an extra life.

Floor 10 keeps the same composable grammar but makes anchors, halls, corridors, and standard rooms larger; closets remain small as contrast nooks. Some non-critical filler may therefore be dropped when geometry is tight. Its treasure target is denser and placed in two-item compositions, and its secret budget is one higher, making the finale visibly more rewarding without reverting to loose item scatter.

## 4. Carving and connections

- **Room interiors**: floor codes taken from the district's sound zone (see §6). `_carve_notches` occasionally removes a mirrored pair or mirrored four-corner set, reserves the resulting anchors, and requires decoration to furnish them with matching accents. The old outward alcove-carving pass is gone. `_add_pillars` provides rarer symmetric structural landmarks where the room tier permits them.
...
- **Secrets**: `_place_secret` and `_carve_secret_pocket` push a wall into a bespoke square, vault, reliquary, gallery, or nested pocket reachable only through its pushwall. The complete footprint and its one-tile shell must be unused wall, so a secret cannot leak into a normally accessible room. Ordinary pockets have three deliberately spaced rewards; floor 9 turns every pocket into a seven-item boss-preparation cache with four ammo clips, first-aid, a machine gun or chaingun, and a premium slot that may contain the floor's extra life. Required boss solvency still lives on the normal route, so secrets remain exciting advantages rather than mandatory progression. Pockets never break outer-wall integrity; there is a run-time check in `validate_map`.
...
Each eligible concept first attempts a recognizable signature: matched bunks in barracks, spear racks in armories, wall-backed appliances in kitchens, cages in jails, wall-mounted armor displays in war rooms, barrels in storage, and restrained equivalents for the other concepts. Suit-of-armor statics are accepted only on a room's perimeter with a solid, non-door wall directly behind them, including armor selected for landmark frames. Kitchens are a floor-level set piece capped at one, not the default treatment for every lounge. A stove establishes the room; the sink is independently optional, and any sink, stove, kitchen supplies, and pots are spaced around wall-backed positions rather than emitted as one repeated clump. Pots and pans remain kitchen-only rather than serving as universal filler. The broader registered WL6 static range supplies the concept vocabulary; object 63 (`CallApogee`) is excluded and validation rejects it defensively. Atmosphere filters clean versus grim props without changing topology.
...
|------------------------|-------------:|--------------------|
| Rooms that are a plain rectangle | 18.1% | `_carve_notches` gives a restrained minority of eligible rooms mirrored corner bites; the outward alcove pass was removed because it produced an overly recognizable silhouette. |
...
Every RNG call goes through a per-floor `random.Random` seeded from the campaign seed and floor number. Variant and circulation selection (§2.1) use independent streams, so these campaign-scale choices neither perturb nor are perturbed by candidate retries. Given the same version, seed, and settings the generator produces byte-identical PK3s. The manifest records the resolved seed, floor seeds, variant, circulation skeleton, district modes, layout signature, room concepts, key order, secret shapes, and pickup compositions.

`validate_map`, `validate_objects`, and `validate_door_axes` run after every floor and reject: shallow or visually direct elevator routes, discontinuous critical routes, unreachable exits, out-of-order or redundant key gates, secret lock bypasses, invalid circulation hierarchy, corridor mediation outside its quality band, pickups without exact provenance, floor tiles outside the sound-zone range, disconnected floor components, doors with mismatched axis tiles, and actor codes outside `ENEMY_CODES`. These checks protect the qualities players actually notice—fair progress, readable layouts, and worthwhile exploration—not just file validity. `tools/fuzz.py` and `tools/smoke_ecwolf.py` provide broader seed and engine coverage.
