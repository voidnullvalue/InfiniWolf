...
This document describes how the generator turns a seed into a Wolfenstein 3D floor: the floor-plan grammar, room placement, corridor and door carving, the map analyses that inform later passes, and how enemies and pickups are placed. These systems work together to make seeds varied, readable, and fun to explore rather than merely valid. Start with the end-to-end diagram in [`GENERATION_FLOW.md`](GENERATION_FLOW.md), then use this document for the rationale and implementation details. Everything here reflects the generation package; function names are included so readers can jump straight to source. The pipeline is split into `infiniwolf/tiles.py` (the native WL6 code vocabulary), `infiniwolf/mapgrid.py` (room records and tile-plane primitives), `infiniwolf/placement.py` (the geometry-aware composition engine shared by the pickup and decoration passes), `infiniwolf/decorations.py` (static decoration), and `infiniwolf/generator.py` (everything else, plus the `generate_map` pipeline itself). Each module imports only from the ones above it in that list.
