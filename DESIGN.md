...
Each floor is generated under a named **base variant** — one frozen bundle of the parameters that used to be module constants: the carve chances (`_carve_notches`/`_carve_alcoves`/`_add_pillars`), corridor widening, the plan grammar's hall/closet/motif rolls, the allowed `WALL_THEMES` bases, the cellblock probability, and the decoration density and theme leanings. Five variants rotate on floors 1–8 (`garrison`, `catacombs`, `grand-halls`, `storehouse`, `quarters`); floors 9 and 10 are the forced `stronghold`/`vault` identities whose parameters stay at the defaults because those floors keep their purpose-built inline treatments (§3).
...
Decoration placement follows theming and is anchored to room features (`_room_anchors`) rather than scattered. Blocking arrangements try, in priority order: a matched furniture pair framing a landmark wall (the cell in front of the picture stays clear), the rare partial divider, a corner stash cluster with a spill of loose pots/blood beside it, the grand colonnade, a matched pair flanking a doorway, a mirrored banquet row down a mess hall's center axis, a courtyard centerpiece at the exact room center, and finally mirrored pairs drawn only from the wall-hugging band. Every group still commits through the full-map reachability check, doorway entry cells plus one cell straight in are banned to all blocking decor so an entrance never reads furniture-jammed, and the §9.1 statics soft cap is enforced with a live headroom counter. In sufficiently large barracks, guardposts, grand rooms, and lounges, a 75% roll splits the room on its long axis and gives each half a compatible furniture concept whose open clutter prefers cells beside its own cluster, then wall-hugging cells. Open items elsewhere are equally anchored: ceiling fixtures hang only on the center axis, gore and pots sit beside furniture or at wall midpoints, jail cells get a clustered skeleton-and-blood corner, long straight corridors get ceiling lights at a fixed four-tile rhythm, and small dead-end alcove pockets earn one deliberate niche piece at their deepest cell (sealed secret pockets are never decorated).
