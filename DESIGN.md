...
Eligible blue-stone rooms independently have a 35% chance to become a cellblock. A cellblock needs a clean five-tile wall run and paints every two of three wall tiles as barred cells: plain bars are nine times as likely as the skeleton variant, while the remaining blue-stone tile reads as a mortar pillar. Loose remains can sit beside plain bars, and jail decoration pools stay deliberately sparse: barrels, blood, and bone variants only. The selected room indices are shared by the wall and decoration passes so the visual treatment and prop bias cannot disagree.
...
Decoration placement follows theming. In sufficiently large barracks, guardposts, grand rooms, and lounges, a 75% roll splits the room on its long axis and gives each half a compatible furniture concept: for example, mess tables opposite supply clutter. Each cluster still commits through the normal full-map reachability check. Storage and corridors retain their single-purpose scattered treatment, and the existing divider, barrel-cluster, and colonnade compositions remain intact.
