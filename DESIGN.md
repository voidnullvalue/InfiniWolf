...
Each floor is generated under a named **base variant** — one frozen bundle of the parameters that used to be module constants: mirrored-notch and structural-column chances, corridor widening, plan-grammar hall/closet/motif rolls, allowed `WALL_THEMES` bases, cellblock probability, decoration density, and concept leanings. Five variants rotate on floors 1–8 (`garrison`, `catacombs`, `grand-halls`, `storehouse`, `quarters`); floors 9 and 10 are the forced `stronghold`/`vault` identities whose parameters stay at the defaults because those floors keep their purpose-built inline treatments (§3). A non-mixed theme bias gives the selected variant triple weight while retaining contrast floors and the no-immediate-repeat rule.

Selection is a pure function of the campaign seed. `LittleEntropyMachine` is the named source for the independent floor, variant, circulation, and campaign-lock streams; its payload formats are frozen so naming the source does not change established output. `_variant_sequence` and `_circulation_sequence` both exclude immediate repeats, and retries affect only the floor-attempt stream. Retries therefore keep both the floor identity and its high-level building organization. The chosen variant, special family, skeleton, district modes, and compact layout signature are recorded in `GeneratedMap` and the reproducibility manifest.
...
Every realized start room receives an `ArrivalDetail`. `_place_arrival_elevator` reserves a rock-bounded one-cell cab outside the wall opposite the first progression direction, installs the correct east/west or north/south elevator door, and deliberately omits a working switch. The player stands two cells inside the room and uses the matching directional start code, facing into the floor with the elevator behind them. Validation proves there is exactly one player start, a clear forward landing, an inert cab, and no exposed arrival shell.
...
`LittleEntropyMachine` derives every independent seed stream, after which floor-local choices use an isolated `random.Random`. Variant, circulation, lock schedule, and floor-attempt streams cannot perturb one another. Given the same version, seed, and settings the generator produces byte-identical PK3s. The manifest names `LittleEntropyMachine` and records the resolved seed, floor seeds, arrival, encounter templates and families, hidden slots, patrol routes and target, guard recesses, base and special families, circulation skeleton, district modes, layout signature, room shapes and concepts, lighting families, vine screens, key objectives, secret details, special-floor anchors, and pickup compositions.

`validate_map`, `validate_objects`, and `validate_door_axes` run after every floor and reject: invalid arrival cabs or player facing, actors without encounter provenance, consecutive critical-route ambushes, stationary floors that miss a meaningful patrol setting, broken patrol routes, malformed guard recesses, shallow or visually direct elevator routes, discontinuous critical routes, unreachable exits, out-of-order, trivial, or redundant key gates, secret lock bypasses, unbounded or normally exposed secret elevators, asymmetric room profiles, incomplete vine screens, unbacked spear/armor/flag displays, mixed room lighting families, invalid circulation hierarchy, corridor mediation outside its quality band, pickups without exact provenance, floor tiles outside the sound-zone range, disconnected floor components, doors with mismatched axis tiles, and actor codes outside `ENEMY_CODES`. Special-floor validation additionally proves the floor-9 boss/staging/victory contract and the floor-10 source/premium/expedition contract. These checks protect the qualities players actually notice—fair progress, readable layouts, and worthwhile exploration—not just file validity. `tools/fuzz.py` and `tools/smoke_ecwolf.py` provide broader seed and engine coverage.
