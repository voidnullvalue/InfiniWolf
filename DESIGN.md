...
- **Room tiers**: `standard` (6–9 square), `hall` (major side 9–13), `corridor` (major side 8–13 and minor side 3–4), `anchor` (10–13 square, exactly one per floor — normally the climax room), and `closet` (4–5 square). Two or three mandatory spine beats become true `corridor` nodes instead of merely room-shaped connectors.
...
- **Circulation composition**: the floor chooses one of six skeletons (`bent-spine`, `parallel-cross`, `central-wings`, `forked`, `perimeter-loop`, or `staggered-grid`). Each district independently chooses one of six modes (`double-loaded`, `single-loaded`, `suite`, `service-bays`, `formal-axis`, or `tunnel-cluster`). The base variant weights compatible choices but never owns a fixed layout, so a theme cannot become a recognizable topology stamp.
- **Filler**: rooms are added until `target = min(20, 14 + 2 * complexity)`. Suite and tunnel-cluster districts may extend short room chains. Other modes prefer attaching rooms to circulation nodes, producing corridor-mediated offices, bays, and wings instead of a continuous carpet of adjacent rooms.
...
Selection is a pure function of the campaign seed. `LittleEntropyMachine` is the named source for the independent floor, variant, circulation, campaign-lock, vine-sector, and guard-gallery streams. `_variant_sequence` and `_circulation_sequence` both exclude immediate repeats, campaign assembly validates that invariant again, and retries affect only the floor-attempt stream. Retries therefore keep both the floor identity and its high-level building organization. The chosen variant, special family, skeleton, district modes, and compact layout signature are recorded in `GeneratedMap` and the reproducibility manifest.

## 3. Room realisation (`_place_planned_rooms`)

Sizes are drawn from `_room_size` per tier; grouped rooms (e.g. `wings`) share the same drawn size. The start room lands in a randomised quadrant with a `heading` offset from an edge, and every subsequent room is placed adjacent to its parent by `adjacent(...)`. Circulation nodes align their major axis with travel and skeleton-specific turns shape the route; corridor-loaded district modes prefer cross-axis room attachments. This gives each seed a legible floor-plan rhythm without reducing themes to fixed templates. `legal(...)` enforces the 3-tile map margin and pad-2 overlap rule; if a room cannot fit adjacent to its parent, the plan drops it (and its subtree). Dropped rooms and edges are cleaned up before the planner returns a `PlacedPlan`.

Every realized start room receives an `ArrivalDetail`. `_place_arrival_elevator` chooses among five rationalized inactive arrivals: a flush elevator façade, an open empty car behind the player, an open car containing one theme-aware supply, a player start inside an already-open car, or a player start inside a car whose ordinary elevator door begins closed but can be opened. All five use native inert tile 85, which renders the exact same `ELEV1` faces as functional tile 21 without carrying its exit trigger; an opening door therefore does not make the elevator capable of changing floors. Open and closed cars reuse the real elevator's two-tile interior, side rails, back wall, and complete five-by-five rock envelope so elevator textures cannot leak into another space. Floor 10 uses the same weighted choice rather than forcing an inside-car signature. Outside starts face into the floor with the elevator behind them; inside starts face out through the doorway. A staged car's single ammo, health, food, or treasure item follows the floor variant rather than a raw random roll. The planner's N/E/S/W convention is explicitly translated to ECWolf's old-format East/North/West/South player-start order. Validation proves the exact archetype geometry, player orientation, clear threshold, contextual item, inert panels, and bounded shell.

Floor 9 is a dedicated boss-stronghold grammar. Its single 14–17 tile anchor is the mandatory boss arena, preceded by a visible staging room and followed by a calm victory room before the elevator. A seeded family (`throne-stronghold`, `command-bunker`, `laboratory-gauntlet`, `columned-fortress`, or `central-duel`) changes motifs, concepts, and arena character independently of the circulation skeleton and district modes. The arena gets sparse symmetric cover rather than ordinary clutter, its support force is deliberately small, and the staging room receives a named stock-up composition. Hans or Gretel supplies the verified native gold-key drop; an optional silver stage can still precede the fight. Floor-9 secrets are reachable before the arena and become seven-item boss-preparation caches with four ammo clips, first-aid, a weapon, and a premium reward.
...
- **Room interiors**: floor codes taken from the district's sound zone (see §6). `_carve_notches` occasionally removes a mirrored pair or mirrored four-corner set, reserves the resulting anchors, and requires decoration to furnish them with matching accents. `_carve_symmetric_profiles` adds a bounded choice of stepped crosses, paired side bays, and paired end bays. All profiles are bilaterally symmetric and parity-aware; the combined notch/profile budget is at most 25% of realized rooms, no profile family owns more than half that budget, and rectangles remain the clear majority. The old outward alcove-carving pass is gone. `_add_pillars` provides rarer symmetric structural landmarks where the room tier permits them.
...
|------------------------|-------------:|--------------------|
| Rooms that are a plain rectangle | 18.1% | Rectangles intentionally remain the clear generated majority. At most 25% of realized rooms receive a mirrored notch or symmetric stepped/bayed profile; the outward alcove pass was removed because it produced an overly recognizable silhouette. |
...
| Rooms per map (mean) | 24.7 | The compact Wolf3D grid targets `min(20, 14 + 2·complexity)` rooms; optional rooms may be dropped when preserving clean geometry requires it. |
| Doors per map (mean) | 27.2 | Doors emerge from finalized graph connections and choke analysis, with a 56-door safety budget. |
| Door-graph cycles per map (mean) | 1.1–1.54 | The `ring` motif guarantees a real reconvergence while validation prevents it from shortcutting mandatory progression. |
